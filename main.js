...
//const c_mail=document.getElementById('upmail');
let uppass = [];
let inpass = [];
...
function signup() {
    ...
    if (upmail.value.includes('@gmail.com') && uppass.length > 0) {
        sessionStorage.setItem("upname", document.getElementById('upmail').value);
        sessionStorage.setItem("uppass", uppass);
        var myText = "Account Created Succesfully";
        alert(myText);
    }
    else if (upmail.value.includes("") || uppass.length === 0) {
...
function signin() {
   ...
   /* if  ((inmail.value.length <= 0) || (inpass.value.length <= 0)){
        alert("Invalid Email or Password");
...
function NewTab() {
    window.open(
      "https://www.netflix.com/in/", "_blank");
}
